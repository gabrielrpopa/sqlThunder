...
public class UpdateUserTask implements Callable<Boolean> {
    ...
    public UpdateUserTask(final DbConnectionInfo connectionDetailInfo,
                          final User u) {
    ...
    public Boolean call() throws Exception {
        try {
            User usr = UpdateUserTask.getUser(connectionDetailInfo, u.getUser());
            if(usr == null) {
                AppLogger.logError(Thread.currentThread().getStackTrace()[1], AppLogger.db, "user does not exist");
                // user doesn't exist
                return false;
            }
            ...
            if(usr.getUserType().toUpperCase().compareTo("SUPER") == 0
                ...
                List<User> supers = UpdateUserTask.getAllUsersByType(connectionDetailInfo, "SUPER");
                if(supers.size() >= 9 && u.getUserType().compareToIgnoreCase("SUPER") == 0 && usr.getUserType().compareToIgnoreCase("SUPER") !=0) { /*we can have another SUPER*/
                    throw new Exception(AppLogger.logError(Thread.currentThread().getStackTrace()[1], AppLogger.db, "No more SUPER users allowed"));
                } else {
                    return UpdateUserTask.updateSuper(connectionDetailInfo, u);
                }
            } else { /*Regular user can be updated regardless*/
                return UpdateUserTask.quickUserUpdate(connectionDetailInfo, u);
            }
        } catch(Exception ex) {
            AppLogger.logException(ex, Thread.currentThread().getStackTrace()[1], AppLogger.db);
            return false;
        }
    }
    ...
    UpdateUserDistributed(final ConcurrentMap<String, SqlRepoDatabase> dbs, final User u) {
        boolean ret = true;
        ...
        for (SqlRepoDatabase db: dbs.values()) {
            try {
                ...
                Callable<Boolean> task = new UpdateUserTask(dbConn, u);
    ...
    quickUserUpdate(final DbConnectionInfo connectionDetailInfo,
        ...
        try (Connection conn = DriverManager.getConnection(connectionDetailInfo.getDbUrl(),connectionDetailInfo.getUserName(), connectionDetailInfo.getPassword());
            ...
            preparedStatement.setInt(2, s.getDepartment());
            preparedStatement.setInt(3, s.getTitle());
            preparedStatement.setInt(4, s.getManager());
