...
public class WebSocketsWrapper {
    ...
    sendSingleMessageToUserFromServer(	final WebsocketPayload message) {
        try {
            boolean b = RestApiWebSocket.sendToUpdatesToSpecificUser(message, message.getBaseUrl());
            System.out.println("sendSingleMessageToUserFromServer: " + b  + ":" + message.getBaseUrl());
        } catch(Exception ex) {
