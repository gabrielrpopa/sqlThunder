...
public class GetUserTitleTask implements Callable<Title> {
    ...
    public static Title
    getTitleDistributed(final ConcurrentMap<String, SqlRepoDatabase> dbs, final long id) {
        String methodName = Thread.currentThread().getStackTrace()[1].getMethodName();
    ...
    public static Title
    getTitle(final DbConnectionInfo connectionDetailInfo, final long id) throws Exception {
        String methodName = Thread.currentThread().getStackTrace()[1].getMethodName();
