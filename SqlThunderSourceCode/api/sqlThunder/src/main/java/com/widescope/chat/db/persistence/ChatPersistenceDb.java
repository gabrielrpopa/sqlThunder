...
public class ChatPersistenceDb {

    public static final
    String chat_external_user_table_postgres = "CREATE TABLE IF NOT EXISTS chat_external_user (id BIGINT  GENERATED BY DEFAULT AS IDENTITY(START WITH 1) PRIMARY KEY, "
    ...
    public static final
    String chat_user = "CREATE TABLE IF NOT EXISTS chat_user (id BIGINT GENERATED BY DEFAULT AS IDENTITY(START WITH 1) PRIMARY KEY, "
                                                    + "	fromId BIGINT, "
    ...
    public static final
    String chat = "CREATE TABLE IF NOT EXISTS chat (id BIGINT GENERATED BY DEFAULT AS IDENTITY(START WITH 1) PRIMARY KEY, "
                                                + "	fromUser VARCHAR(MAX), "
                                                + "	toUser VARCHAR(MAX), "
                                                + "	timestamp BIGINT, "
    ...
    public static final
    String chat_group = "CREATE TABLE IF NOT EXISTS chat_group (id BIGINT GENERATED BY DEFAULT AS IDENTITY(START WITH 1) PRIMARY KEY, " +
                                                                "groupName BIGINT )";

    public static final
    String chat_group_users = "CREATE TABLE IF NOT EXISTS chat_group_users (id BIGINT GENERATED BY DEFAULT AS IDENTITY(START WITH 1) PRIMARY KEY, groupId BIGINT, userId BIGINT )";
    ...
    createAllTables(DbConnectionInfo dbConn) throws Exception{
        try {
            ...
            lst.add(ChatPersistenceDb.chat_user);
            ...
            ChatPersistenceDb.createTables(dbConn, lst);
    ...
    createTable(final DbConnectionInfo connectionDetailInfo, final String stm) throws Exception	{
        ...
        Statement statement;
        try (Connection conn = DriverManager.getConnection(connectionDetailInfo.getDbUrl(),
                                                            connectionDetailInfo.getUserName(),
                                                            connectionDetailInfo.getPassword()) ) {
    ...
    getChatUserTo(final DbConnectionInfo connectionDetailInfo, final String userName) throws Exception {
        Class.forName(connectionDetailInfo.getJdbcDriver());
        String sqlString = "SELECT id, fromId, fromUser, isFromExt, toId, toUser, isToExt FROM chat_user WHERE toUser = ? ";
        ChatUser ret = new ChatUser();
        ...
        try (Connection conn = DriverManager.getConnection(connectionDetailInfo.getDbUrl(),connectionDetailInfo.getUserName(), connectionDetailInfo.getPassword());
            ...
            ResultSet rs = preparedStatement.executeQuery();
            if ( rs.next() ) {
                ret = new ChatUser(  rs.getLong("fromId"),
                        rs.getString("fromUser"),
                        rs.getString("isFromExt"),
                        rs.getLong("toId"),
                        rs.getString("toUser"),
    ...
    getUserToChat(final DbConnectionInfo connectionDetailInfo, final String email) throws Exception {
        Class.forName(connectionDetailInfo.getJdbcDriver());
        String sqlString = "SELECT id, firstName, lastName, email, department, title, manager, characteristic, description, nodeId FROM chat_external_user WHERE email LIKE ? ";
        List<UserToChat> ret = new ArrayList<>();
        ...
        try (Connection conn = DriverManager.getConnection(connectionDetailInfo.getDbUrl(),connectionDetailInfo.getUserName(), connectionDetailInfo.getPassword());
            ...
            while ( rs.next() ) {
                ...
                UserToChat b = new UserToChat(  rs.getLong("id"),
