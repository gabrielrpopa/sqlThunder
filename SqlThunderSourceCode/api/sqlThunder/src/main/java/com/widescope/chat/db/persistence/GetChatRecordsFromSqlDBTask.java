    ...
    public List<ChatRecord> call() throws Exception {
        try {
            if(isDelivered!=null && fromDate == 0 && toDate == 0)
                return ChatPersistenceRef.getMessages(connectionDetailInfo, fromUser, toUser, isDelivered);
            else if(isDelivered==null && fromDate > 0 && toDate > 0) {
                return ChatPersistenceRef.getMessageHist(connectionDetailInfo, fromUser, toUser, fromDate, toDate);
            } else {
                return ChatPersistenceRef.getMessageHist(connectionDetailInfo, fromUser, toUser, fromDate, toDate);
            }
        } catch(Exception ex) {
