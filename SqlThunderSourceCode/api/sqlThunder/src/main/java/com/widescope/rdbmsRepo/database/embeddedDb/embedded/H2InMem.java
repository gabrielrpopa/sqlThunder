...
public class H2InMem implements  EmbeddedInterface {
	...
	public H2InMem(	final long _clusterId, 
					...
					final String user_) throws Exception	{
	...
	getTableList(final String schemaName) throws Exception 	{
		...
		try	{
			...
			DbUtil.closeHandles(conn, null, null);
	    } 
		return ret;
	}
	...
	@Override
	public synchronized
	ResultQuery execStaticQueryToWebsocket(final User u,
		...
		DbConnectionInfo connectionDetailInfo = DbConnectionInfo.makeH2ConnectionInfo(dbRelativePath, clusterId, USER, PASS);
	...
	public
	synchronized
	boolean bulkInsert(	final List<RowValue> tableData) throws Exception {
		boolean isOK = false;
		Statement statement = null;
		try {
			statement = conn.createStatement();
	...
	insertBulkIntoEmbeddedTable(final String schemaName,
								final String tableName,
								List<RowValue> tableData) throws Exception {
		RecordsAffected ret = new RecordsAffected("INSERT", 0, 0);
		Connection conn = getConnection();
		Map<String, String> metadata =  SqlMetadataWrapper.generateMetadata(tableData);
		try	{
			String sqlStm = SqlMetadataWrapper.generateInsertTableStm(metadata, schemaName, tableName);
			Statement statement = conn.createStatement();
			for(RowValue row: tableData) {
				Map<String, Object>  rowT = SqlMetadataWrapper.generateMRows(row);
				String execInsert = SqlMetadataWrapper.generateExecutableInsertTableStm(sqlStm, metadata, rowT);
				try {
					int recordsAffected = statement.executeUpdate(execInsert);
					ret.addRecAffected(recordsAffected);
				} catch(Exception ex) {
	...
	public DataTransfer loadRdbmsQueriesInMem(	final List<RdbmsTableSetup> resultList, boolean isClosing) throws Exception {
		boolean isCreated = createUserTableOnce(resultList.get(0).getCreateTableStm());
		DataTransfer ret = new DataTransfer();
		if(!isCreated) return ret;
		for(RdbmsTableSetup t: resultList) {
			...
			if(isClosing) {
				...
				DbUtil.closeHandles(null, statement, null);
			}
		}
		EmbeddedWrapper.addInMemEmbeddedQuery(this, "in-mem query execution by aggregating multiple datasets");
		ret.setIsSuccess(true);
