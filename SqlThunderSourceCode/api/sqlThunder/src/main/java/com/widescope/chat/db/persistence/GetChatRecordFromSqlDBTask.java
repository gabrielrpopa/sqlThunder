    ...
    public ChatRecord call() throws Exception {
        try {
            if(this.operation.compareToIgnoreCase("GET") == 0)
                return ChatPersistenceDb.getMessage(connectionDetailInfo, fromUser, toUser, requestId);
            else if(this.operation.compareToIgnoreCase("SET") == 0)
                return ChatPersistenceDb.setReadMessage(connectionDetailInfo, fromUser, toUser, requestId);
            else
                return ChatPersistenceDb.deleteMessage(connectionDetailInfo, fromUser, toUser, requestId);
