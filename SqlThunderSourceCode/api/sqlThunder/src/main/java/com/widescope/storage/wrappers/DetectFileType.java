...
public class DetectFileType implements RestInterface {
	...
	public static String detectMimeType(final InternalFileStorageRecord r) {
		if(r.getMimeType().equalsIgnoreCase("application/pdf")) {
			return "PDF";
		} else if(r.getMimeType().equalsIgnoreCase("audio/aac")) { // AAC audio (.aac)
			return "AUDIO";
		} else if(r.getMimeType().equalsIgnoreCase("application/x-abiword")) { // AbiWord document (.abw)
			return "WORD";
		} else if(r.getMimeType().equalsIgnoreCase("image/avif")) { // AVIF image  (.avif)
			return "IMAGE";
		} else if(r.getMimeType().equalsIgnoreCase("application/vnd.ms-excel")) { // Microsoft Excel  (.xls)
			return "EXCEL";
		} else if(r.getMimeType().equalsIgnoreCase("application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")) { // Microsoft Excel  (.xlsx)
			return "EXCEL";
		} else if(r.getMimeType().equalsIgnoreCase("text/csv")) { // .csv
			return "CSV";
		} else if(r.getMimeType().equalsIgnoreCase("application/msword")) { // .doc
			return "WORD";
		} else if(r.getMimeType().equalsIgnoreCase("application/vnd.openxmlformats-officedocument.wordprocessingml.document")) { // .doc
			return "WORD";
		} else {
			return "UNKNOWN";
		}
