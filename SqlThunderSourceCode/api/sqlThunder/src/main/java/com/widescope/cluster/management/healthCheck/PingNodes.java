...
public class PingNodes {
	...
	static
	List<MachineNode>
	queryAllNodesInfoFromPingResult(final List<PingResult> lst) {
	...
	static
	List<MachineNode>
	queryMachinesInfo(final List<MachineNode> lst) {
		if(lst == null || lst.isEmpty()) return new ArrayList<>();
		List<MachineNode> ret = new ArrayList<MachineNode>();
		List<Future<MachineNode>> list = new ArrayList<Future<MachineNode>>();
		ExecutorService executor = Executors.newFixedThreadPool(lst.size());
	...
	public static void printMachineInfo(List<MachineNode> discoveredMachineList) {
		System.out.println("-------------- Machine Nodes Info -------------------");
		int maxLength = discoveredMachineList.stream().map(MachineNode::getBaseUrl).toList().stream().mapToInt(String::length).max().orElse(0); /*get max length*/
		discoveredMachineList.forEach(
				x-> {
					String baseUrl = String.format("%-" + maxLength + "s", x.getBaseUrl());
					System.out.println(baseUrl + "," + x.getType() + " " + x.getIsAccepted());
				}
		);
	}


	public static void addDiffMachines(List<MachineNode> source, List<MachineNode> minus) {
		List<MachineNode> m = source.stream().filter(item -> !minus.contains(item)).toList();
		ClusterDb clDb = new ClusterDb();
		m.forEach(machine -> {
            try {
                clDb.addNode(machine.getBaseUrl(), machine.getType(), machine.getIsAccepted(), machine.getIsRegistered());
            } catch (Exception ex) {
				AppLogger.logException(ex, Thread.currentThread().getStackTrace()[1], AppLogger.obj);
            }
