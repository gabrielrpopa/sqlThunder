...
public final class ResponseHandler {
     ...
     */
    public ModelAndView handleTerminalTransitions(	AuthenticationResponse response, 
    												HttpSession session,
    												final String issuer) {
        Util.updateSession(session, response.getProceedContext());
        if (response.getTokenResponse() != null) {
            return homeHelper.proceedToHome(response.getTokenResponse(), session, issuer);
        }

        if (response.getAuthenticators() == null && !response.getErrors().isEmpty()) {
        ...
        if (response.getAuthenticationStatus() == SKIP_COMPLETE) {
            ModelAndView modelAndView = homeHelper.proceedToHome(response.getTokenResponse(), session, issuer);
     ...
     */
    public ModelAndView handleKnownTransitions(AuthenticationResponse response, 
    											HttpSession session,
    											final String issuer) {
        ModelAndView terminalModelAndView = handleTerminalTransitions(response, session, issuer);
    ...
    public ModelAndView selectAuthenticatorForm(AuthenticationResponse response, String title, HttpSession session) {
        String issuer = ""; // the issuer url
		String clientId = ""; // the client id
		String clientSecret = ""; // the client secret
        Set<String> scopes = new HashSet<String>(); // the set of scopes
        String redirectUri = ""; // the redirect uri
        
		IDXAuthenticationWrapper authenticationWrapper 
