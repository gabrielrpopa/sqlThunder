...
public class AuthUtil {
	...
	public static boolean isSessionAuthenticated_(String user, String session, String uri) {
		boolean isAuthenticated = false;
		...
		if(0 == StaticApplicationProperties.userSessionCacheLocation) { // internal cache
			if(InternalUserDb.loggedUsers.containsKey(session))	{
				/*implement timeout session*/
				return InternalUserDb.validateAndClearSession(user, session, uri);
			}
			return false;
		}
	...
	public boolean deleteDepartment(final String name) throws Exception {
		try	{
			if(!InternalUserDb.dataWhales.getSqlDbRefsList().isEmpty()) {
				return DeleteUserDeptByNameTask.deleteDeptByNameDistributed(InternalUserDb.dataWhales.getSqlDbRefsList(), name);
			} else {
				internalUserDb.getDepartment(name);
				return internalUserDb.deleteDepartment(name);
			}
		} catch(Exception ex)	{
			AppLogger.logException(ex, className, Thread.currentThread().getStackTrace()[1].getMethodName(), AppLogger.obj);
			return false;
