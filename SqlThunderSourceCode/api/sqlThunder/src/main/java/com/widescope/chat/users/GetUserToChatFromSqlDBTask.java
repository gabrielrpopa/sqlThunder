...
import com.widescope.chat.db.persistence.ChatPersistenceDb;
...
public class GetUserToChatFromSqlDBTask implements Callable<List<UserToChat>> {
    ...
    public List<UserToChat> call() throws Exception {
        try {
            //List<ChatUser> uList = ChatPersistenceDb.getUsersMinusUser(connectionDetailInfo, likeUser, fromUser);
            return ChatPersistenceDb.getUserToChat(connectionDetailInfo, fromUser);
        } catch(Exception ex) {
