...
public class EmbeddedWrapper {
	...
	loadH2ClusterInMem(	final long clusterId,
		...
		EmbeddedInMemCluster c = new EmbeddedInMemCluster(clusterId,  sessionId, requestId, user, comment);
		for(EmbeddedDbRecord embDb: r.getEmbeddedDbRecordList() ) {
			H2InMem h2InMem = loadH2DatabaseInMem(clusterId,embDb, sessionId, requestId, user );
	...
	loadCsvInMem(	final EmbeddedDbRecord e,
		...
		ClusterTransfer ret = new ClusterTransfer();
		EmbeddedInMemCluster c = new EmbeddedInMemCluster(-1,  sessionId, requestId, user, comment);
	...
	loadH2ClusterInMem(	final long clusterId, 
		...
		ClusterTransfer ret = new ClusterTransfer();
		EmbeddedInMemCluster c = new EmbeddedInMemCluster(clusterId,  sessionId, requestId, user, comment);
		H2InMem h2InMem = loadH2DatabaseInMem(clusterId,e, sessionId, requestId, user );
	...
	loadH2DatabaseInMem(final long clusterId,
		...
		H2InMem h2InMem = new H2InMem(clusterId, embDb.getDbId(), "TABLE", sessionId, requestId, user);
