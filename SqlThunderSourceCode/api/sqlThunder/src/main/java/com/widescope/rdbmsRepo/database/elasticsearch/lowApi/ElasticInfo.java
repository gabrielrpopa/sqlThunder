...
public class ElasticInfo {
	...
	getIndexMappings(	ElasticLowLevelWrapper elasticLowLevelWrapper,
		...
		try {
			...
			try {
				...
				String indexBodyStr = json.get(indexName).toString();
				JSONObject indexBodyJson = (JSONObject) parser.parse(indexBodyStr);
				String mappingsStr = indexBodyJson.get("mappings").toString();
				
				JSONArray propertiesJson = (JSONArray) parser.parse(mappingsStr);

                for (Object o : propertiesJson) {
	...
	executeGeneric(ElasticLowLevelWrapper elasticLowLevelWrapper,
		...
		if(isOriginalFormat.equalsIgnoreCase("Y")) {
			try {
				Response response = elasticLowLevelWrapper.getRestClient().performRequest(request);
				return (Object) parser.parse(EntityUtils.toString(response.getEntity()));
			} catch(Exception ex) {
				throw new Exception(AppLogger.logException(ex, Thread.currentThread().getStackTrace()[1], AppLogger.obj)) ;
			}
		} else {
