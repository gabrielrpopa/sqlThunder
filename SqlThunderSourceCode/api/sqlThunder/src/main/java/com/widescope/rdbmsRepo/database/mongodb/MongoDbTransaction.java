...
public class MongoDbTransaction {
    ...
    public static void addBucket(final String clusterUniqueName,
        ...
        MongoDbTransaction.deleteInitialFile(clusterUniqueName, databaseName, bucketName);
    ...
    public static String  addIndexToCollection(final String clusterUniqueName,
                                            final String databaseName,
        ...
        MongoDbConnection mongoDbConnection = new MongoDbConnection(mongoClusterRecord.getConnString(),
                mongoClusterRecord.getClusterId(),
                mongoClusterRecord.getUniqueName());
        indexName = MongoPut.createIndex(mongoDbConnection, databaseName, collectionName, fieldName);
        mongoDbConnection.disconnect();
        return indexName;
    }
    ...
    public static long
    deleteRecords(final String mongoClusterName,
                  final String mongoDatabaseName,
                  final String mongoCollectionName,
        ...
        MongoDbConnection mongoDbConnection = new MongoDbConnection(mongoClusterRecord.getConnString(),
                                                                    mongoClusterRecord.getClusterId(),
                                                                    mongoClusterRecord.getUniqueName());
        long ret = MongoPut.deleteRecords(mongoDbConnection, mongoDatabaseName, mongoCollectionName, commandStr);
        mongoDbConnection.disconnect();
        return ret;
    ...
    public static Document getDocument(final String clusterUniqueName,
                                       final String databaseName,
                                       final String collectionName,
                                       final String docId) throws Exception {
        MongoClusterRecord mongoClusterRecord = SqlRepoUtils.mongoDbMap.get(clusterUniqueName);
        MongoDbConnection mongoDbConnection = new MongoDbConnection(mongoClusterRecord);
        Document ret = MongoGet.getDocument(mongoDbConnection, databaseName, collectionName, docId);
        mongoDbConnection.disconnect();
        return ret;
    }
    ...
    public static long deleteDocFromCollection(final String clusterUniqueName,
                                               final String databaseName,
                                               final String collectionName,
                                               final String docId) throws Exception {
        MongoClusterRecord mongoClusterRecord = SqlRepoUtils.mongoDbMap.get(clusterUniqueName);
    ...
    public static long deleteManyDocsFromCollection(final String clusterUniqueName,
                                                    final String databaseName,
                                                    final String collectionName,
    ...
    public static String addFileToBucket(final MongoClusterDb mongoClusterDb,
        ...
        LargeObjectAssociatedMetadata metadata_;

        try {
            metadata_ = LargeObjectAssociatedMetadata.toLargeObjectAssociatedMetadata(metadata);
        } catch(Exception ignored) {
            metadata_ = new LargeObjectAssociatedMetadata();
        }

        if(metadata_ == null) { metadata_ = new LargeObjectAssociatedMetadata(); }
        metadata_.setUserId( userId );
        ...
        String fileId =
                MongoBucket.addFileToBucket(	mongoClusterDb,
                                                mongoDbConnection,
                                                databaseName,
                                                bucketName,
                                                fileName,
    ...
    public static LargeMongoBinaryFile getFileFromBucket(final String clusterUniqueName,
                                                         final String databaseName,
                                                         final String bucketName,
    ...
    public static boolean deleteFileFromBucket(final String clusterUniqueName,
                                               final String databaseName,
                                               final String bucketName,
                                               final String fileId) throws Exception {
        MongoClusterRecord mongoClusterRecord = SqlRepoUtils.mongoDbMap.get(clusterUniqueName);
        MongoDbConnection mongoDbConnection = new MongoDbConnection(mongoClusterRecord.getConnString(),
                                                                    mongoClusterRecord.getClusterId(),
                                                                    mongoClusterRecord.getUniqueName());
        boolean ret = MongoBucket.deleteFileFromBucket(mongoDbConnection, databaseName, bucketName, fileId);
        mongoDbConnection.disconnect();
        return ret;
    }

    public static boolean deleteManyFilesFromBucket(final String clusterUniqueName,
                                                   final String databaseName,
                                                   final String bucketName,
        ...
        MongoDbConnection mongoDbConnection = new MongoDbConnection(mongoClusterRecord.getConnString(),
                                                                    mongoClusterRecord.getClusterId(),
                                                                    mongoClusterRecord.getUniqueName());
        boolean ret = MongoBucket.deleteManyFilesFromBucket(mongoDbConnection, databaseName, bucketName, filter);
        mongoDbConnection.disconnect();
        return ret;
    }

    public static boolean deleteInitialFile(final String clusterUniqueName,
                                                    final String databaseName,
                                                    final String bucketName) throws Exception {
        MongoClusterRecord mongoClusterRecord = SqlRepoUtils.mongoDbMap.get(clusterUniqueName);
        MongoDbConnection mongoDbConnection = new MongoDbConnection(mongoClusterRecord.getConnString(),
                                                                    mongoClusterRecord.getClusterId(),
                                                                    mongoClusterRecord.getUniqueName());
        boolean ret = MongoBucket.deleteManyFilesFromBucket(mongoDbConnection, databaseName, bucketName, "{\"filename\": \"toBeDeleted.null\"}");
        mongoDbConnection.disconnect();
        return ret;
    }
