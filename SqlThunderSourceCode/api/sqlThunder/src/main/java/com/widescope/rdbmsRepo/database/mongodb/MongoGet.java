...
import com.widescope.rdbmsRepo.database.SqlRepository.Objects.MongoDbId;
...
public class MongoGet {
	...
	public static Document getDocument(final MongoDbConnection mongoDbConnection,
									   final String dbName,
									   final String collectionName,
									   final String id) {
		try	{
			final Document theDoc = new Document("_id", new ObjectId(id));
			MongoDatabase database = mongoDbConnection.getMongoClient().getDatabase(dbName);
			FindIterable<Document> docCursor = database.getCollection(collectionName).find(theDoc);
	...
	public static String getDocumentById(	final MongoDbConnection mongoDbConnection,
		...
		try	{
			...
			for(Document doc: docCursor) {
				...
				resultQuery.setMongoDbId(new MongoDbId(id, collectionName, dbName, mongoDbConnection.getClusterName()));
				resultQuery.setResultQuery(doc.toJson());
				return resultQuery.toString();
			}
			return new ResultQuery().toString();
		}
		catch(Exception ex)	{
			AppLogger.logException(ex, Thread.currentThread().getStackTrace()[1], AppLogger.obj);
			return null;
	...
	public static MongoResultSet getDocumentById_(final MongoDbConnection mongoDbConnection,
											 final String dbName,
											 final String collectionName,
											 final String id) {
		try	{
			...
			FindIterable<Document> docCursor = database.getCollection(collectionName).find(theDoc);
			for(Document doc: docCursor) {
				MongoResultSet mongoResultSet = new MongoResultSet(doc);
				mongoResultSet.setCountCollection(maxCountCollection);
	...
	getAllElements(final MongoDbConnection mongoDbConnection,
				   final String dbName,
				   final String collectionName) {
		List<Document> ret = new ArrayList<>();
		MongoDatabase database = mongoDbConnection.getMongoClient().getDatabase(dbName);
		FindIterable<Document> docCursor = database.getCollection(collectionName).find(new Document());
		for(Document doc: docCursor) {
			String objectId = doc.getObjectId("_id").toHexString();
			doc.remove("_id");
			doc.append("_id", objectId);
	...
	/**
	 * Example jsonString = "{ columnName: { $in: [ "val1", "val2" ] } }"  similar to WHERE column in ("val1", "val2")
	...
	rawQueryCollection(final MongoDbConnection mongoDbConnection,
						final String dbName,
						final String collectionName,
						final String query) {
		BasicDBObject dbObj = BasicDBObject.parse(query);
		MongoDatabase database = mongoDbConnection.getMongoClient().getDatabase(dbName);
		FindIterable<Document> docCursor = database.getCollection(collectionName).find(dbObj);
		...
		return new MongoResultSet(lst, lst.size(), count);
	}
	...
	searchDocumentComplexAnd(	final MongoDbConnection mongoDbConnection, 
		...
		try	{
			...
			for(Map.Entry<String, Range> elem: complexAndSearch.getRange().entrySet()) {
				BasicDBObject rangeQuery = new BasicDBObject(elem.getKey(),  BasicDBObjectBuilder.start("$gte", elem.getValue().getFrom_()).add("$lte", elem.getValue().getTo_())   ) ;
				andObj.add(rangeQuery);
			}
			
			for(Map.Entry<String, List<Object> > elem: complexAndSearch.get_in().entrySet()) {
	...
	public static <T> MongoResultSet searchDocumentParallel(final String user,
															final ComplexAndSearch complexAndSearch)  {
		MongoResultSet ret = new MongoResultSet();
		try	{
			int maxThreads = 0;
	...
	public static <T> MongoResultSet queryResults(  String user,
													int sqlId, 
													long fromDateTime, 
		...
		ComplexAndSearch complexAndSearch = new ComplexAndSearch(new HashMap<String, Range>(),
											...
											0,
											0);
		return searchDocumentParallel(user, complexAndSearch);
	}
