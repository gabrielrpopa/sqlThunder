...
public class ChatPersistence {
    ...
    public static long maxNumberDocPerCollection = 2147483648L;
    public static long maxCollNo = 999L;


    public void addChatGroup(ChatDb chatDb,
                             AuthUtil authUtil,
                             final long fromId,
        ...
        if(!InternalUserDb.dataWhales.getSqlDbRefsList().isEmpty()) {
            ...
            ChatPersistenceRef.insertUserTable(dbConn,fromId,fromUser,toId, toUser);
    ...
    public String addMessage(ChatDb chatDb,
        ...
        if(!InternalUserDb.dataWhales.getMongoClusterList().isEmpty()) {
            ...
            if(size < maxSizeMessage - 2048) {
                ...
                ret = MongoDbTransaction.addChatMessageToCollection(mapMongoConn.get(0).getUniqueName(), ChatPersistence.mongoDatabase, ChatPersistence.mongoCollection, cMessage, requestId);
                ...
                ret = MongoDbTransaction.addLargeChatMessageToBucket(  mapMongoConn.get(0).getUniqueName(),
            ...
            ChatFileService chatFileService = new ChatFileService(appConstants.getChatStoragePath());
        ...
        if(!InternalUserDb.dataWhales.getSqlDbRefsList().isEmpty()) {
            ...
            try {
                ChatPersistenceRef.mergeChat(dbConn, chatRecord);
                ret = "Y";
            } catch(Exception ex) {
                ret = "N";
            }
    ...
    public ChatMessage
    getMessageFromStore_(ChatDb chatDb,
                        AppConstants appConstants,
    ...
    deleteMessage(ChatDb chatDb,
        ...
        if(chatRecord.getMongoUniqueName() != null && chatRecord.getMongoDatabase() != null && chatRecord.getMongoCollection() != null) {
            MongoClusterRecord  mapMongoConn = InternalUserDb.dataWhales.getMongoClusterList().get(chatRecord.getMongoUniqueName());
            m = MongoDbTransaction.deleteChatMessageById(mapMongoConn.getUniqueName(), ChatPersistence.mongoDatabase, ChatPersistence.mongoCollection, chatRecord.getRequestId());
        } else {
            ChatFileService chatFileService = new ChatFileService(appConstants.getChatStoragePath());
            m = chatFileService.readMessage(chatRecord.getFromUser(), chatRecord.getToUser(), chatRecord.getRequestId());
    ...
    public long getUreadMessagesCount(ChatDb chatDb, final String toUser) throws Exception {
        long count = 0;
        if(!InternalUserDb.dataWhales.getSqlDbRefsList().isEmpty()) {
