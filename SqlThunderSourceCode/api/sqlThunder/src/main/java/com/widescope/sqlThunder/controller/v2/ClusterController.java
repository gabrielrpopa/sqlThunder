...
public class ClusterController {
	...
	getNodeById(@RequestHeader(value="requestId", defaultValue = "") String requestId,
		...
		requestId = StringUtils.generateRequestId(requestId);
		try	{
			MachineNode c = clusterDb.getNode(id);
	...
	addNode(@RequestHeader(value="requestId", defaultValue = "") String requestId,
		...
		try	{
			...
			String msg = "Machine cannot be added or already exists";
			return RestObject.retException(new GenericResponse(msg), requestId, Thread.currentThread().getStackTrace()[1].getMethodName(), msg);
		}

		try {
			c.setIsPong(Objects.requireNonNull(RestApiCluster.ping(c.getBaseUrl())));
		} catch(Exception ex)	{
			String msg = "Machine cannot be reached";
			return RestObject.retException(new GenericResponse(msg), requestId, Thread.currentThread().getStackTrace()[1].getMethodName(), msg);
		}
	...
	public ResponseEntity<RestObject>
	testAdminAccount (@RequestHeader(value="internalAdmin") final String internalAdmin,
					  @RequestHeader(value="internalAdminPassword") final String internalAdminPassword,
					  @RequestHeader(value="requestId", defaultValue = "") String requestId) {

		if( !authUtil.isInternalAdminAuthenticated(internalAdmin, internalAdminPassword) )
			return RestObject.retOKWithPayload(new GenericResponse("OK"), requestId, Thread.currentThread().getStackTrace()[1].getMethodName());
		else
			return RestObject.retOKWithPayload(new GenericResponse("ERROR"), requestId, Thread.currentThread().getStackTrace()[1].getMethodName());
	}
	...
	public ResponseEntity<RestObject>
	testUserAccount (@RequestHeader(value="internalUserName") final String internalUserName,
					 @RequestHeader(value="internalUserPassword") final String internalUserPassword,
					 @RequestHeader(value="requestId", defaultValue = "") String requestId) {
		if( authUtil.isInternalUserAuthenticated(internalUserName, internalUserPassword))
			return RestObject.retOKWithPayload(new GenericResponse("OK"), requestId, Thread.currentThread().getStackTrace()[1].getMethodName());
		else
			return RestObject.retOKWithPayload(new GenericResponse("ERROR"), requestId, Thread.currentThread().getStackTrace()[1].getMethodName());
	}
	...
	@CrossOrigin(origins = "*")
	@RequestMapping(value = "/cluster/node:ping", method = RequestMethod.GET)
	...
	ping(@RequestHeader(value="requestId", defaultValue = "") String requestId,
		...
		try	{
			...
			return RestObject.retOKWithPayload(new GenericResponse("NOT_REACHED"), requestId, Thread.currentThread().getStackTrace()[1].getMethodName());
		}
	}
	...
	public ResponseEntity<RestObject>
	pong (@RequestHeader(value="requestId", defaultValue = "") String requestId) {
		return RestObject.retOKWithPayload(new GenericResponse("PONG"), requestId, Thread.currentThread().getStackTrace()[1].getMethodName());
	}
